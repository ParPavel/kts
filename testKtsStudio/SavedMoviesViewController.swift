...
class SavedMoviesViewController: UITableViewController , NSFetchedResultsControllerDelegate {
    ...
    func configureCell(_ cell: UITableViewCell, withEvent event: Movie) {
        cell.textLabel!.text = event.tittle
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let film = self._fetchedResultsController?.fetchedObjects?[indexPath.row] else { return }
        let overview = film.value(forKey: "overview") as! String
        let dataImage = film.value(forKey: "data") as! Data
        let title = film.value(forKey: "tittle") as! String
        
        
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        guard let contentVC = storyboard.instantiateViewController(withIdentifier: "savedСontentViewController") as? SavedСontentViewController else {
            return
        }
        contentVC.overviewText = overview
        contentVC.dataImage = dataImage
        contentVC.nameFilm = title
